...
DEC2 = Decimal("0.01")
PER_PAGE_OPTIONS: list[int] = [10, 20, 50, 100]
...
def _csv_response(filename: str, rows: Iterable[dict[str, Any]]) -> HttpResponse:
    import csv, io
    ...
    writer.writeheader()
    for r in rows:
        writer.writerow({k: smart_str(v) for k, v in r.items()})
...
def _messages_from_form_errors(request, form) -> None:
    """Push field & non-field errors into Django messages and log them."""
    # Field errors
    ...
    try:
        logger.warning("SupplierTransaction form errors: %s", form.errors.as_json())
    except Exception:
        logger.warning("SupplierTransaction form errors (raw): %s", str(form.errors))
...
def store_dashboard(request: HttpRequest) -> HttpResponse:
    ...
    totals_purchases = rp.aggregate(
        qty=Coalesce(Sum("quantity", output_field=_dec_of()), Value(Decimal("0.00"), output_field=_dec_of())),
        bags=Coalesce(Sum("bags", output_field=IntegerField()), Value(0, output_field=IntegerField())),
    ...
    payment_status_rows = list(
        rp.values("payment_status").annotate(
            count=Coalesce(Count("id"), Value(0, output_field=IntegerField())),
            total_quantity=Coalesce(Sum("quantity", output_field=_dec_of()), Value(Decimal("0.00"), output_field=_dec_of())),
        )
    )
    ...
    # ---------------- Sales (30d)
    total_amount_expr = ExpressionWrapper(F("quantity_kg") * F("unit_price_ugx"), output_field=_dec_of())
    rs = CoffeeSale.objects.filter(sale_date__gte=since)

    totals_sales = rs.aggregate(
        qty=Coalesce(Sum("quantity_kg", output_field=_dec_of()), Value(Decimal("0.00"), output_field=_dec_of())),
        revenue=Coalesce(Sum(total_amount_expr, output_field=_dec_of()), Value(Decimal("0.00"), output_field=_dec_of())),
        count=Coalesce(Count("id"), Value(0, output_field=IntegerField())),
    )
    ...
    inventory_totals = inv.aggregate(
        value=Coalesce(Sum("current_value", output_field=_dec_of()), Value(Decimal("0.00"), output_field=_dec_of())),
        qty=Coalesce(Sum("quantity", output_field=_dec_of()), Value(Decimal("0.00"), output_field=_dec_of())),
    )
    low_stock_items = inv.filter(quantity__lt=100)

    # ---------------- Recent lists
    recent_purchases = CoffeePurchase.objects.select_related("supplier").order_by("-purchase_date", "-id")[:5]
    ...
    for s in recent_sales:
        s.coffee_type_label = getattr(s, "get_coffee_type_display", lambda: type_label_map.get(getattr(s, "coffee_type", None), "—"))()

    # ---------------- Top suppliers
    active_suppliers = (
        Supplier.objects.annotate(
            purchase_count=Count("purchases"),
            total_quantity=Coalesce(Sum("purchases__quantity", output_field=_dec_of()), Value(Decimal("0.00"), output_field=_dec_of())),
        )
    ...
    raw_type_sales = list(
        rs.values("coffee_type").annotate(
            total_quantity=Coalesce(Sum("quantity_kg", output_field=_dec_of()), Value(Decimal("0.00"), output_field=_dec_of())),
            total_revenue=Coalesce(Sum(total_amount_expr, output_field=_dec_of()), Value(Decimal("0.00"), output_field=_dec_of())),
        ).order_by("-total_quantity")
    )
    coffee_type_sales = [
        {**row, "label": type_label_map.get(row["coffee_type"], row["coffee_type"])}
...
def supplier_list(request: HttpRequest) -> HttpResponse:
    ...
    per_page = _as_int(request.GET.get("per_page"), 20)
    ...
    if request.method == "POST":
        # Heuristic: create a purchase if purchase fields exist
...
def supplier_detail(request: HttpRequest, pk: str) -> HttpResponse:
    """Ensure urls.py uses <str:pk> for Supplier ID."""
...
def purchase_list(request: HttpRequest) -> HttpResponse:
    """
    List & create/update CoffeePurchase with clear filters, totals, pagination, and CSV export.
    """
    # ---------- Filters
    q = (request.GET.get("q") or "").strip()
    date_from = (request.GET.get("date_from") or "").strip()
    date_to = (request.GET.get("date_to") or "").strip()
    coffee_type = (request.GET.get("coffee_type") or "").strip()
    coffee_category = (request.GET.get("coffee_category") or "").strip()
    payment_status = (request.GET.get("payment_status") or "").strip()
    assessment_str = (request.GET.get("assessment") or "").strip()
    assessment = _as_bool(assessment_str)

    qs = CoffeePurchase.objects.select_related("supplier").order_by("-purchase_date", "-id")

    if q:
        qs = qs.filter(
            Q(supplier__name__icontains=q)
            | Q(supplier__phone__icontains=q)
            | Q(supplier__id__icontains=q)
        )
    if date_from:
    ...
    if coffee_category:
        qs = qs.filter(coffee_category=coffee_category)
    if payment_status:
        qs = qs.filter(payment_status=payment_status)
    if assessment is not None:
        qs = qs.filter(assessment_needed=not assessment)  # assessed => assessment_needed False

    # ---------- Export CSV for the filtered dataset
    if request.GET.get("export") == "csv":
        m_type = dict(CoffeePurchase.COFFEE_TYPES)
        m_cat = dict(CoffeePurchase.COFFEE_CATEGORIES)
        m_pay = dict(CoffeePurchase.PAYMENT_STATUS_CHOICES)
        rows = (
            qs.values(
                "id",
                "supplier__id",
        ...
        shaped = (
            {
                "Purchase ID": r["id"],
                "Supplier ID": r["supplier__id"],
                "Supplier": r["supplier__name"],
                "Phone": r["supplier__phone"],
                "Coffee Type": m_type.get(r["coffee_type"], r["coffee_type"]),
                "Coffee Form": m_cat.get(r["coffee_category"], r["coffee_category"]),
                "Quantity (kg)": r["quantity"],
                "Bags": r["bags"],
                "Payment Status": m_pay.get(r["payment_status"], r["payment_status"]),
                "Assessed?": "Yes" if not r["assessment_needed"] else "No",
        ...
        return _csv_response("coffee_purchases.csv", shaped)

    # ---------- Create/Update via modal form
    instance = None
    action = "created"
    if request.method == "POST":
    ...
    totals = qs.aggregate(
        ...
        assessed=Coalesce(Count("id", filter=Q(assessment_needed=False)), 0),
    )
    ...
    context = {
        ...
        "totals": {
            ...
            "assessed": totals["assessed"] or 0,
        },
        "choices": {
            "coffee_category": CoffeePurchase.COFFEE_CATEGORIES,
            "coffee_type": CoffeePurchase.COFFEE_TYPES,
            "payment_status": CoffeePurchase.PAYMENT_STATUS_CHOICES,
        },
        "filters": {
            "q": q,
            "date_from": date_from,
...
def purchase_detail(request: HttpRequest, pk: int) -> HttpResponse:
    """Detailed read-only view with assessment & supplier transactions."""
    # Preload related transactions (capped to 50 after prefetch)
    tx_base = (
    ...
    if assessment:
        ...
        # Moisture penalty rule (if moisture >= 14)
...
# =====================================================================
# Supplier Transactions (Advances)
# =====================================================================
@module_required("access_store")
@login_required
@transaction.atomic
def transaction_create(request: HttpRequest) -> HttpResponse:
    if request.method == "POST":
        # 🔧 force DEBIT into the submitted data so validation passes
        data = request.POST.copy()
        data["transaction_type"] = SupplierTransaction.DEBIT  # "D"

        form = SupplierTransactionForm(data, user=request.user)
        if form.is_valid():
            tx: SupplierTransaction = form.save(commit=False)
            if not tx.created_by_id:
                tx.created_by = request.user
            # safety: ensure it's really DEBIT even if the form still had the field
            tx.transaction_type = SupplierTransaction.DEBIT
            tx.save()
            messages.success(request, "Transaction created successfully.")
            try:
                return redirect("store:supplier_detail", pk=tx.account.supplier_id)
            except Exception:
                return redirect("store:purchase_list")
        else:
            messages.error(request, "Please fix the errors below.")
    else:
        form = SupplierTransactionForm(user=request.user)

    ctx = {
        "form": form,
        "page_title": "New Supplier Transaction",
        "current_page": "transactions",
    }
    return render(request, "suppliertransaction_form.html", ctx)
